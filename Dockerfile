...
RUN case "$PREINSTALL_LIBS" in \
        ...
        *) \
            echo -e "Invalid value for build argument INCLUDE_POSTGRESQL_TOOLS"; \
            exit 1;; \
    esac
...
RUN case "$INCLUDE_MONGODB_TOOLS" in \
        ...
        *) \
            echo "Invalid value for build argument INCLUDE_MONGODB_TOOLS"; \
            exit 1;; \
    esac
...
RUN case "$INCLUDE_MARIADB_TOOLS" in \
        ...
        *) \
            echo -e "Invalid value for build argument INCLUDE_MARIADB_TOOLS"; \
            exit 1;; \
    esac
